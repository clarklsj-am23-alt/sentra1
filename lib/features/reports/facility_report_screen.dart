...
class _FacilityReportScreenState extends State<FacilityReportScreen> {
  ...
  String _selectedIssueType = 'Out of Service';
  ...
  void initState() {
    ...
    _selectedStation = widget.initialStation ?? stations.first;

    _selectedFacilityType = widget.initialFacilityType ?? facilityTypes.first;
  ...
  Future<void> _loadReports() async {
    ...
    try {
      final data = await _reportService.getFacilityReports();
      ...
      _showError('Unable to load facility reports.');
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _submitReport() async {
    ...
    if (_selectedStation == null || _selectedFacilityType == null) {
      _showError('Please select station and facility.');
      return;
    }

    if (description.isEmpty) {
      _showError('Please describe the facility problem.');
      return;
    }

    if (description.length < 5) {
      _showError('Please provide a clearer description.');
      return;
    }
    ...
    try {
      await _reportService.addFacilityReport(
      ...
      _showSuccess('Facility report submitted successfully.');

      await _loadReports();
    } catch (e) {
      _showError('Unable to submit report. Please try again.');
    } finally {
      if (mounted) {
        setState(() {
          _submitting = false;
  ...
  Future<void> _deleteReport(Map<String, dynamic> report) async {
    ...
    try {
      await _reportService.deleteFacilityReport(report['id']);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Report Facility Problem')),
          ...
          children: [
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Example: Lift near Exit A is not working.',
            ...
            if (_loadingReports)
              ...
              const Card(
                child: Padding(
                  padding: EdgeInsets.all(24),
                  child: Center(child: Text('No facility reports yet.')),
                ),
              )
            else
              ..._reports.map((report) {
